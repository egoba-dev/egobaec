...
class LeftTextBlockRepository extends AbstractRepository
{
    ...
    public function findAllOrderByCreateDate()
    {
        return $this->createQueryBuilder('ltb')
            ->where('ltb.deletedAt IS NULL')  // 論理削除されていないもののみ
            ->orderBy('ltb.createDate', 'DESC')  // create_date → createDate に修正
            ->getQuery()
            ->getResult();
    }
    ...
    public function findVisibleOrderBySortNo()
    {
        return $this->createQueryBuilder('ltb')
            ...
            ->andWhere('ltb.deletedAt IS NULL')  // 論理削除されていないもののみ
            ->setParameter('visible', true)
            ->orderBy('ltb.sortNo', 'ASC')       // sort_no → sortNo に修正
            ->addOrderBy('ltb.createDate', 'ASC') // create_date → createDate に修正
            ->getQuery()
            ->getResult();
    }
    ...
    public function getMaxSortNo()
    {
        $qb = $this->createQueryBuilder('ltb')
            ->select('MAX(ltb.sortNo) as max_sort_no')  // sort_no → sortNo に修正
            ->where('ltb.deletedAt IS NULL');  // 論理削除されていないもののみ
